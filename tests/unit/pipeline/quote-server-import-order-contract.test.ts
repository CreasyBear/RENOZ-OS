...
describe('pipeline quote server import order contract', () => {
  it('keeps quote server dependencies declared before executable code', () => {
    ...
    expect(source).toContain("import { createAdminSupabase } from '@/lib/supabase/server';");
    expect(source).toContain("import { fetchOrganizationForDocument } from '@/server/functions/documents/organization-for-pdf';");
