...
describe("BulkOperations", () => {
  it("disables export and shows an explicit unavailable reason when no export workflow is wired", () => {
    render(
      <BulkOperations
        selectedCount={3}
        ...
        onDelete={vi.fn()}
        canExport={false}
        exportUnavailableReason="Customer export is unavailable until this route provides an export workflow."
      />
    );

    expect(screen.getByRole("button", { name: /^export$/i })).toBeDisabled();
    expect(screen.getByText(/bulk export unavailable/i)).toBeInTheDocument();
    expect(
      screen.getByText(/customer export is unavailable until this route provides an export workflow/i)
    ).toBeInTheDocument();
  });
