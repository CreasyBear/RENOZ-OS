...
export interface MovementMetadata {
  ...
  /** Reservation expiry for reserved stock */
...
export const inventory = pgTable(
  "inventory",
  {
    ...
    quantityAvailable: numericCasted("quantity_available", {
      precision: 10,
      scale: 3,
    })
      .notNull()
      .generatedAlwaysAs(sql`quantity_on_hand - quantity_allocated`), // onHand - allocated (generated)
  ...
  (table) => ({
    ...
    orgCreatedIdx: index("idx_inventory_org_created").on(
      table.organizationId,
...
export const inventoryMovements = pgTable(
  ...
  (table) => ({
    ...
    // RLS policies for org isolation (append-only: select + insert only)
...
export const stockCounts = pgTable(
  ...
  (table) => ({
    ...
    orgCreatedIdx: index("idx_stock_counts_org_created").on(
      table.organizationId,
      table.createdAt.desc(),
      table.id.desc()
    ),
...
export const inventoryAlerts = pgTable(
  ...
  (table) => ({
    ...
    orgCreatedIdx: index("idx_alerts_org_created").on(
      table.organizationId,
      table.createdAt.desc(),
      table.id.desc()
    ),
...
export type Location = typeof warehouseLocations.$inferSelect;
export type NewLocation = typeof warehouseLocations.$inferInsert;
