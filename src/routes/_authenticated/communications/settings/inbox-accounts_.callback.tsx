...
function InboxEmailAccountsCallbackPage() {
  ...
  useEffect(() => {
    ...
    // Invalid callback payload (missing both error and auth params)
    handledRef.current = true;
    toast.error('Invalid email connection callback', {
      description: 'Missing authorization code. Please try connecting your account again.',
    });
    navigate({
      to: "/communications/settings/inbox-accounts",
      search: {
