/**
 * Stock Counts Page Component
 *
 * Cycle counting management with list, detail, and counting views.
 *
 * @source counts from useStockCounts hook
 * @source countDetail from useStockCount hook
 * @source locations from useLocations hook
 *
 * @see src/routes/_authenticated/inventory/counts.tsx - Route definition
 */
...
export default function StockCountsPage() {
  ...
  type StockCountApiItem = Awaited<ReturnType<typeof listStockCounts>>['counts'][number];
  const counts: StockCount[] = (countsData?.counts ?? []).map((c: StockCountApiItem) => ({
    id: c.id,
  ...
  const countItems: CountItem[] = (countDetailData?.count?.items ?? [])
    ...
    .map((item: StockCountItemWithRelations) => ({
      ...
      unitCost:
        item.inventory?.unitCost !== null && item.inventory?.unitCost !== undefined
