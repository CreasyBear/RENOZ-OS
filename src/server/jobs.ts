 ...
 *
 * @see drizzle/schema/jobs.ts for database schema
 * @see src/lib/schemas/jobs.ts for validation schemas
 */
  ...
  .handler(async ({ data }) => {
    ...
    if (data.createNotification) {
      await db.insert(notifications).values({
        ...
        type: "system",
        title: data.success
          ? `${existingJob.name} completed`
          : `${existingJob.name} failed`,
