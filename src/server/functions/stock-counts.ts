...
type StockCountRecord = typeof stockCounts.$inferSelect;
type StockCountItemRecord = typeof stockCountItems.$inferSelect;

interface StockCountWithItems extends StockCountRecord {
...
interface ListStockCountsResult {
...
export const listStockCounts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<ListStockCountsResult> => {
...
export const updateStockCount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: { id, data } }) => {
    ...
    const [existing] = await db
      ...
      .from(stockCounts)
      .where(
        and(eq(stockCounts.id, id), eq(stockCounts.organizationId, ctx.organizationId))
      )
      .limit(1);
    ...
    const [count] = await db
      ...
      .set({
        ...
        ...(data.varianceThreshold !== undefined && { varianceThreshold: String(data.varianceThreshold) }),
...
export const bulkUpdateCountItems = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [count] = await db
      ...
      .where(
        and(
          eq(stockCounts.id, data.countId),
          eq(stockCounts.organizationId, ctx.organizationId)
        )
      )
      .limit(1);
  ...
  .handler(async ({ data }) => {
    ...
    if (uncountedItems.length > 0) {
      throw new ValidationError(
        `${uncountedItems.length} items have not been counted`,
        {
    ...
    return await db.transaction(async (tx) => {
      ...
      const varianceItems = items.filter(
        (i) => i.countedQuantity !== i.expectedQuantity
      );
...
export const getCountVarianceAnalysis = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const totalCounted = varianceItems.reduce(
      (sum, i) => sum + (i.countedQuantity ?? 0),
      0
    );
    const totalValueImpact = itemsWithVariance.reduce(
      (sum, i) => sum + i.valueImpact,
      0
    );
      ...
      summary: {
        ...
        accuracyRate:
          varianceItems.length > 0
            ? Math.round(
                ((varianceItems.length - itemsWithVariance.length) /
        ...
        return {
          ...
          accuracyRate:
            stats && stats.totalItems > 0
              ? Math.round(
                  ((stats.totalItems - stats.varianceItems) / stats.totalItems) * 100
                )
              : 100,
        };
      })
