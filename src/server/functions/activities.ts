...
export const getActivityFeed = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { cursor, pageSize, entityType, entityId, action, userId, source, dateFrom, dateTo } = data;
...
export const requestActivityExport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const ctx = await withAuth({ permission: PERMISSIONS.activity?.export ?? "customer.export" });
    ...
    return {
      ...
      message: "Activity export is not yet available. This feature is planned for a future release.",
    };
  });
...
export const getRecentActivityCount = createServerFn({ method: "GET" })
  .inputValidator(
    activityStatsQuerySchema.pick({ dateFrom: true, dateTo: true }).optional()
  )
  .handler(async ({ data }) => {
    const ctx = await withAuth({ permission: PERMISSIONS.activity?.read ?? "customer.read" });
