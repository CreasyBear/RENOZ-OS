...
interface ReceiveManualInventoryInput {
  organizationId: string;
  userId: string;
  data: ReceiveInventoryInput;
}

export async function receiveManualInventory({
  organizationId,
  userId,
  data,
}: ReceiveManualInventoryInput) {
  ...
  return await db.transaction(async (tx) => {
    ...
    const [costLayer] = await tx
      ...
      .where(
        and(eq(inventoryCostLayers.id, costLayerId), eq(inventoryCostLayers.organizationId, organizationId))
      )
      .limit(1);
