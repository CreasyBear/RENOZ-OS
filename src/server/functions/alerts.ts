...
export const listAlerts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<ListAlertsResult> => {
    ...
    if (filters.triggered) {
      ...
      conditions.push(
        sql`${inventoryAlerts.lastTriggeredAt} > NOW() - INTERVAL '24 hours'`
      );
    }
...
export const getTriggeredAlerts = createServerFn({ method: "GET" })
  .handler(async (): Promise<{ alerts: TriggeredAlert[]; count: number }> => {
    ...
    for (const alert of activeAlerts) {
      const triggered = await checkAlertTriggered(ctx.organizationId, alert);
      if (triggered) {
...
export const checkAndTriggerAlerts = createServerFn({ method: "POST" })
  .handler(async () => {
    ...
    for (const alert of activeAlerts) {
      const result = await checkAlertTriggered(ctx.organizationId, alert);
      if (result) {
      ...
      if (threshold.minQuantity !== undefined) {
        const items = await db
          ...
          .where(
            and(
              ...invConditions,
              lt(inventory.quantityOnHand, threshold.minQuantity)
            )
          );
        ...
        if (items.length > 0) {
          ...
          severity = currentValue === 0 ? "critical" : currentValue < thresholdValue / 2 ? "high" : "medium";
    ...
    case "overstock":
      if (threshold.maxQuantity !== undefined) {
        const overstock = await db
          ...
          .innerJoin(products, eq(inventory.productId, products.id))
          .where(
            and(...invConditions, gt(inventory.quantityOnHand, threshold.maxQuantity))
          );
...
function generateAlertRecommendations(
  ...
  const totalInactive = Object.values(byType).reduce(
    (sum, s) => sum + (s.total - s.active),
    0
  );
  if (totalInactive > 5) {
    recommendations.push({
