...
import { costLayerFilterSchema, createCostLayerSchema } from "@/lib/schemas/inventory";
...
export const getInventoryAging = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const bucketData = bucketLabels.map((label, index) => {
      ...
      const totalQuantity = itemsInBucket.reduce(
...
function generateAgingRecommendations(
  ...
  const oldBuckets = bucketData.filter(
    (b) => b.bucket.includes(">") || b.bucket.includes("365")
  );
  ...
  if (oldValue > 0) {
    recommendations.push({
      type: "slow_moving",
      message: `$${oldValue.toFixed(2)} in inventory over 1 year old - consider markdown or disposal`,
      priority: "high",
    });
  }
...
export const getInventoryTurnover = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      byProduct: (turnoverByProduct as unknown as Array<{
        productId: string;
        productSku: string;
        productName: string;
        inventoryValue: number;
