/**
 * Financial dashboard read models.
 */
...
function calculateKPI(current: number, previous?: number): KPIMetric {
  ...
  if (p !== undefined && p !== 0) {
    ...
    metric.changePercent = Number.isNaN(change)
      ? 0
...
export async function readFinancialDashboardMetrics(
  ctx: SessionContext,
  data: z.infer<typeof financialDashboardQuerySchema>,
  ...
  const cashReceivedMTD = await getCashRevenueByDateRange(
    ctx.organizationId,
    monthStartStr,
    monthEndStr,
  );
  const cashReceivedPrevMTD = await getCashRevenueByDateRange(
    ctx.organizationId,
    prevMonthStartStr,
    prevMonthEndStr,
  );
  const cashReceivedYTD = await getCashRevenueByDateRange(
    ctx.organizationId,
    yearStartStr,
    nowStr,
  );
  ...
  const paymentSubquery = db.$with('p').as(
    db
      .select({
        ...
        lastPaymentDate: sql<Date>`MAX(${orderPayments.paymentDate})`.as(
          'last_payment_date',
        ),
      })
      .from(orderPayments)
  ...
  const metrics: FinancialDashboardMetrics = {
    ...
    paymentRate:
      totalPeriodInvoices > 0 ? paidPeriodInvoices / totalPeriodInvoices : 0,
    overdueRate:
      totalPeriodInvoices > 0 ? overduePeriodInvoices / totalPeriodInvoices : 0,
...
export async function readRevenueByPeriod(
  ctx: SessionContext,
  data: z.infer<typeof revenueByPeriodQuerySchema>,
  ...
  const [invoicedResults, cashResults] = await Promise.all([
    ...
    db
      ...
      .groupBy(
        sql`DATE_TRUNC(${truncInterval}, ${orderPayments.paymentDate}::date)`,
      )
      .orderBy(
        sql`DATE_TRUNC(${truncInterval}, ${orderPayments.paymentDate}::date)`,
      )
...
export async function readTopCustomersByRevenue(
  ctx: SessionContext,
  data: z.infer<typeof topCustomersQuerySchema>,
  ...
  if (basis === 'cash') {
    ...
    if (data.dateFrom) {
      dateConditions.push(
        gte(
          ...
          data.dateFrom.toISOString().split('T')[0],
        ),
      );
    }
    if (data.dateTo) {
      dateConditions.push(
        lte(orderPayments.paymentDate, data.dateTo.toISOString().split('T')[0]),
      );
    }
    ...
    const customerRevenue = await db
      ...
      .orderBy(
        desc(
          sql`COALESCE(SUM(CASE WHEN ${orderPayments.isRefund} THEN -${orderPayments.amount} ELSE ${orderPayments.amount} END), 0)`,
        ),
      )
      .limit(pageSize)
      .offset(offset);
    ...
    const outstandingByCustomer = new Map(
      outstandingResult.map((r) => [
        r.customerId,
        safeNumber(r.outstandingAmount),
      ]),
    );
    ...
    const customers: TopCustomerEntry[] = customerRevenue
      .filter(
        (c) =>
          !data.commercialOnly ||
          Number(c.totalRevenue) >= COMMERCIAL_THRESHOLD,
      )
      .map((c) => ({
  ...
  const customers: TopCustomerEntry[] = customerRevenue
    .filter(
      (c) =>
        !data.commercialOnly || Number(c.totalRevenue) >= COMMERCIAL_THRESHOLD,
    )
    .map((c) => ({
      customerId: c.customerId,
      customerName: c.customerName,
...
export async function readOutstandingInvoices(
  ctx: SessionContext,
  data: z.infer<typeof outstandingInvoicesQuerySchema>,
  ...
  return {
    ...
    summary: {
      ...
      averageInvoiceValue:
        totalInvoiceCount > 0 ? totalOutstanding / totalInvoiceCount : 0,
      averageResidentialValue:
        residentialCount > 0 ? residentialOutstanding / residentialCount : 0,
      averageCommercialValue:
        commercialCount > 0 ? commercialOutstanding / commercialCount : 0,
    },
    total: countResult[0]?.count ?? 0,
    page,
    pageSize,
