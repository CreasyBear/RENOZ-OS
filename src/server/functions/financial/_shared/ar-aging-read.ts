...
export async function readARAgingReport(
  ctx: SessionContext,
  data: z.infer<typeof arAgingReportQuerySchema>,
): Promise<ARAgingReportResult> {
  ...
  const paginatedCustomers: CustomerAgingSummary[] = customerSummaryResult.map(
    (row) => {
      ...
      return {
        ...
        oldestInvoiceDate: row.oldest_order_date
          ? new Date(row.oldest_order_date)
          : null,
  ...
  const commercialResult = await db.execute<{ commercial_total: string }>(sql`
      ...
      `);
  const commercialOutstanding = safeNumber(
    commercialResult[0]?.commercial_total,
  );
  ...
  const invoiceCount = Object.values(bucketTotals).reduce(
    (sum, b) => sum + b.count,
    0,
  );
...
export async function readARAgingCustomerDetail(
  ctx: SessionContext,
  data: z.infer<typeof arAgingCustomerDetailQuerySchema>,
): Promise<CustomerAgingDetailResult> {
  const asOfDate = data.asOfDate ? new Date(data.asOfDate) : new Date();
  const asOfDateStr = asOfDate.toISOString().split('T')[0]; // YYYY-MM-DD format
  const { page, pageSize } = data;
  const offset = (page - 1) * pageSize;
