...
export const getSegmentsWithStats = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const segmentsWithStats: SegmentWithStats[] = ([...segments] as any[]).map(
      (row) => ({
        id: row.id,
        name: row.name,
...
export const getSegmentAnalytics = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const customersByStatus = Object.entries(statusCounts).map(
      ([status, count]) => ({
        status,
        count,
    ...
    const totalValue = customersData.reduce(
      (sum, c) => sum + Number(c.lifetime_value),
      0
    )
    const avgHealth =
      customersData.reduce((sum, c) => sum + Number(c.health_score), 0) /
      (total || 1)
