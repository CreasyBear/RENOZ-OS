...
const ATTRIBUTE_TYPES = ['text', 'number', 'boolean', 'select', 'multiselect', 'date'] as const;

type AttributeType = (typeof ATTRIBUTE_TYPES)[number];

// Value type based on attribute type
type AttributeValueType = string | number | boolean | string[] | null;
...
export const updateAttribute = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<ProductAttribute> => {
    ...
    const [existing] = await db
      ...
      .from(productAttributes)
      .where(
        and(eq(productAttributes.id, id), eq(productAttributes.organizationId, ctx.organizationId))
      )
      .limit(1);
