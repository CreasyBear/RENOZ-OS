    ...
    const [category, images, priceTiers, attributeVals, relations, bundleComponents] =
      await Promise.all([
        ...
        db
          ...
          .where(
            and(eq(productPriceTiers.productId, data.id), eq(productPriceTiers.isActive, true))
          )
        ...
        db
          ...
          .where(and(eq(productRelations.productId, data.id), eq(productRelations.isActive, true)))
...
export const updateProduct = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Product> => {
    ...
    const [updated] = await db.update(products).set(setData).where(eq(products.id, id)).returning();
...
export const getCategoryTree = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  const allCategories = await db
    .select()
    .from(categories)
    .where(and(eq(categories.organizationId, ctx.organizationId), eq(categories.isActive, true)))
    .orderBy(asc(categories.sortOrder), asc(categories.name));
