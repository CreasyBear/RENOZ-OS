...
interface BundleWithComponents {
  bundleProduct: Pick<
    Product,
    'id' | 'sku' | 'name' | 'basePrice' | 'type' | 'status' | 'description'
  >;
...
export const updateBundleComponent = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<ProductBundle> => {
    ...
    const [existing] = await db
      .select()
      .from(productBundles)
      .where(and(eq(productBundles.id, id), eq(productBundles.organizationId, ctx.organizationId)))
      .limit(1);
...
export const removeBundleComponent = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<{ success: boolean }> => {
    ...
    const [existing] = await db
      ...
      .from(productBundles)
      .where(
        and(eq(productBundles.id, data.id), eq(productBundles.organizationId, ctx.organizationId))
      )
      .limit(1);
  ...
  .handler(
    async ({
      ...
      const componentProducts = await db
        ...
        .from(products)
        .where(
          and(inArray(products.id, componentIds), eq(products.organizationId, ctx.organizationId))
        );
      ...
      for (const component of bundleComponents) {
        ...
        if (product.type === 'bundle') {
          warnings.push(
            `Component ${component.componentProductId} is itself a bundle (nested bundles)`
          );
        }
      }
