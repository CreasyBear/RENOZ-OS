...
export const listForecasts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<ListForecastsResult> => {
    ...
    if (filters.dateFrom) {
      conditions.push(gte(inventoryForecasts.forecastDate, filters.dateFrom.toISOString().split("T")[0]));
    }
    if (filters.dateTo) {
      conditions.push(lte(inventoryForecasts.forecastDate, filters.dateTo.toISOString().split("T")[0]));
    }
...
export const getProductForecast = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const forecasts = await db
      ...
      .orderBy(asc(inventoryForecasts.forecastDate))
      .limit(Math.ceil(data.days / (data.period === "daily" ? 1 : data.period === "weekly" ? 7 : 30)));
...
export const upsertForecast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const ctx = await withAuth({ permission: "inventory.forecast" });
  ...
  .handler(async ({ data }) => {
    ...
    const avgAccuracy =
      withAccuracy.length > 0
        ? withAccuracy.reduce((sum, f) => sum + Number(f.forecastAccuracy), 0) /
          withAccuracy.length
        : null;
    ...
    const trend = Object.entries(monthlyTrend)
      .map(([month, data]) => ({
        ...
        averageAccuracy:
          data.withAccuracy > 0 ? data.totalAccuracy / data.withAccuracy : null,
      }))
...
async function getHistoricalDemand(
  ...
  return {
    ...
    totalDemand,
    averageDailyDemand:
      demandData.length > 0 ? totalDemand / demandData.length : 0,
  };
}
...
function getZScore(serviceLevel: number): number {
  ...
  for (let i = 0; i < keys.length - 1; i++) {
    if (serviceLevel >= keys[i] && serviceLevel <= keys[i + 1]) {
      ...
      return zScores[String(keys[i])] + ratio * (zScores[String(keys[i + 1])] - zScores[String(keys[i])]);
    }
  }
