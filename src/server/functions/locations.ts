...
export const updateLocation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: { id, data } }) => {
    ...
    const [existing] = await db
      ...
      .from(locations)
      .where(
        and(eq(locations.id, id), eq(locations.organizationId, ctx.organizationId))
      )
      .limit(1);
...
export const deleteLocation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [location] = await db
      ...
      .from(locations)
      .where(
        and(eq(locations.id, data.id), eq(locations.organizationId, ctx.organizationId))
      )
      .limit(1);
...
const warehouseLocationListQuerySchema = z.object({
  parentId: z.string().uuid().optional().nullable(),
  locationType: z.enum(["warehouse", "zone", "aisle", "rack", "shelf", "bin"]).optional(),
...
const updateWarehouseLocationSchema = createWarehouseLocationSchema.partial();
 ...
 */
export const getLocationUtilization = createServerFn({ method: "GET" })
  .handler(async () => {
    const ctx = await withAuth();
