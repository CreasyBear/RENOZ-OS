...
export const createShipment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ShipmentWithItems> => {
    ...
    const shipmentNumber =
      data.shipmentNumber ||
      (await generateShipmentNumber(ctx.organizationId, data.orderId));

    // Generate tracking URL if carrier and tracking number provided
    const trackingUrl =
      data.trackingUrl ||
      generateTrackingUrl(data.carrier ?? null, data.trackingNumber ?? null);
...
export const markShipped = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<OrderShipment> => {
    ...
    const trackingUrl =
      data.trackingUrl ||
      generateTrackingUrl(data.carrier, data.trackingNumber ?? null);
...
export const confirmDelivery = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<OrderShipment> => {
    ...
    trackingEvents.push({
      ...
      status: "delivered",
      description: data.signedBy
        ? `Delivered and signed by ${data.signedBy}`
        : "Delivered",
    });
