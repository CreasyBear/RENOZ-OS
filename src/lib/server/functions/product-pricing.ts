...
export const resolvePrice = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<PriceResolutionResult> => {
      ...
      const [customerPrice] = await db
        ...
        .where(
          and(
            ...
            or(
              isNull(customerProductPrices.validTo),
              gte(customerProductPrices.validTo, now)
            )
          )
        )
    ...
    const [volumeTier] = await db
      ...
      .where(
        and(
          ...
          or(
            isNull(productPriceTiers.maxQuantity),
            gte(productPriceTiers.maxQuantity, quantity)
          )
        )
      )
...
export const deleteCustomerPrice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ success: boolean }> => {
    ...
    await db
      ...
      .where(eq(customerProductPrices.id, data.id));
...
export async function recordPriceChange(params: {
  ...
  productId: string;
  changeType: "base_price" | "cost_price" | "tier_created" | "tier_updated" | "tier_deleted" | "customer_price" | "bulk_update";
...
export const applyPriceAdjustment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<BulkPriceUpdateResult> => {
    ...
    if (data.productIds.length > MAX_BATCH_SIZE) {
      throw new ValidationError(
        `Batch size exceeds limit of ${MAX_BATCH_SIZE} items. Please split into smaller batches.`,
        { productIds: [`Maximum ${MAX_BATCH_SIZE} products allowed, received ${data.productIds.length}`] }
      );
    }
    ...
    for (const productId of data.productIds) {
      try {
        ...
        if ((data.applyTo === "base" || data.applyTo === "both") && product.basePrice) {
          const newBasePrice = Math.round((product.basePrice * multiplier) * roundFactor) / roundFactor;
          ...
          await recordPriceChange({
            ...
            reason: data.reason ?? `${data.adjustmentPercent > 0 ? '+' : ''}${data.adjustmentPercent}% adjustment`,
            changedBy: ctx.user.id,
          });
        ...
        if ((data.applyTo === "cost" || data.applyTo === "both") && product.costPrice) {
          const newCostPrice = Math.round((product.costPrice * multiplier) * roundFactor) / roundFactor;
