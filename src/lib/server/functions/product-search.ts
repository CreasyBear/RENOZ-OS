  ...
  .inputValidator(
    z.object({
      ...
      attributes: z
        ...
        .optional(),
      sortBy: z.enum(["relevance", "name", "price_asc", "price_desc", "created"]).default("relevance"),
...
export const getRelatedProducts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    // Similar price range
    conditions.push(
      gte(products.basePrice, sourceProduct.basePrice - priceRange)
    );
    conditions.push(
      lte(products.basePrice, sourceProduct.basePrice + priceRange)
    );
