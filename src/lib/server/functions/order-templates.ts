...
export const listTemplates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<ListTemplatesResult> => {
    ...
    const { search, isActive, category, page, pageSize, sortBy, sortOrder } =
      data;
    ...
    if (category) {
      conditions.push(
        sql`${orderTemplates.metadata}->>'category' = ${category}`
      );
    }
...
export const createOrderFromTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    for (const templateItem of templateItems) {
      ...
      const discount =
        (templateItem.discountPercent
          ? lineSubtotal * (templateItem.discountPercent / 100)
          : 0) + (templateItem.discountAmount ?? 0);
    ...
    const currentUsage =
      (template.metadata as Record<string, unknown>)?.usageCount ?? 0;
    await db
      .update(orderTemplates)
      .set({
        metadata: {
...
export const deleteTemplateItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ success: boolean }> => {
    ...
    await db
      .delete(orderTemplateItems)
      .where(eq(orderTemplateItems.id, data.itemId));
