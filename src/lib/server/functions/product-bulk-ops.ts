...
export const parseImportFile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const validatedRows: Array<{ row: number; data: ImportRow; isValid: boolean; errors: string[] }> = [];
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      for (const [key, value] of Object.entries(rawRow)) {
        ...
        if (normalizedKey === "tags" && value) {
          normalizedRow[normalizedKey] = value.split(",").map((t) => t.trim()).filter(Boolean);
        } else {
...
export const importProducts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const categoryNames = [...new Set(data.rows.filter((r) => r.categoryName).map((r) => r.categoryName!))];
