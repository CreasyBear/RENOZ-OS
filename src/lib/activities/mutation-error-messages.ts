...
const DEFAULT_CODE_MESSAGES: Record<string, string> = {
  NOT_FOUND: 'Activity target could not be found. Refresh and try again.',
  PERMISSION_DENIED: 'You do not have permission to log activity for this record.',
  AUTH_ERROR: 'Your session has expired. Sign in again before logging activity.',
  RATE_LIMIT: 'Too many activity updates were attempted. Wait a moment and retry.',
};

export const ACTIVITY_MUTATION_MESSAGES = {
  logEntity: 'Activity logging is temporarily unavailable. Please refresh and try again.',
} as const;

export type ActivityMutationAction = keyof typeof ACTIVITY_MUTATION_MESSAGES;
...
export function formatActivityMutationError(
  error: unknown,
  action: ActivityMutationAction
): string {
  const fallback = ACTIVITY_MUTATION_MESSAGES[action];
