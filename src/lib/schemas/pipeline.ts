...
export const opportunityMetadataSchema = z
  .object({
    source: z
      .enum(["referral", "website", "cold_call", "trade_show", "other"])
      .optional(),
...
export const createOpportunitySchema = z.object({
  ...
  value: z.coerce.number().nonnegative().default(0), // In cents
...
export const opportunityListQuerySchema = paginationSchema.merge(
...
export const quoteLineItemSchema = z.object({
  ...
  unitPriceCents: z.number().int().nonnegative(), // Price in cents
  discountPercent: z.number().min(0).max(100).optional(),
  totalCents: z.number().int().nonnegative(), // Line total in cents
});
...
export const pipelineMetricsSchema = z.object({
  ...
  byStage: z.record(opportunityStageSchema, z.object({
    count: z.number(),
    value: z.number(),
    weightedValue: z.number(),
  })),
