...
function FieldDiff({ field, before, after, compact }: FieldDiffProps) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2 py-1 text-sm">
        <span className="text-muted-foreground min-w-[100px] font-medium">{field}:</span>
        {hasChange ? (
          <>
            <span className={cn('rounded px-1.5 py-0.5 line-through', getDiffClasses('before'))}>
              {beforeStr}
            </span>
            <span className="text-muted-foreground">→</span>
            <span className={cn('rounded px-1.5 py-0.5', getDiffClasses('after'))}>{afterStr}</span>
          </>
        ) : (
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] items-start gap-2 border-b py-2 last:border-0">
      <div>
        <div className="text-muted-foreground mb-1 text-xs">Before</div>
        <div className={cn('rounded px-2 py-1 text-sm break-words', getDiffClasses('before'))}>
          {beforeStr}
        </div>
      </div>
      <div className="text-muted-foreground mt-4 self-center">→</div>
      <div>
        <div className="text-muted-foreground mb-1 text-xs">After</div>
        <div className={cn('rounded px-2 py-1 text-sm break-words', getDiffClasses('after'))}>
          {afterStr}
        </div>
      </div>
    </div>
...
export function InlineChangeDiff({
  ...
  return (
    <span className={cn('inline-flex items-center gap-1', className)}>
      <span className={cn('rounded px-1 text-xs line-through', getDiffClasses('before'))}>
        {beforeStr}
      </span>
      <span className="text-muted-foreground text-xs">→</span>
      <span className={cn('rounded px-1 text-xs', getDiffClasses('after'))}>{afterStr}</span>
    </span>
  );
}
