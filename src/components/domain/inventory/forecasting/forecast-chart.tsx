...
export const ForecastChart = memo(function ForecastChart({
  ...
  const maxValue = Math.max(
    ...data.map((d) =>
      Math.max(d.forecastQuantity, d.actualQuantity ?? 0)
    ),
    1
  );
          ...
          <div className="flex items-center gap-4 mt-4 pt-4 border-t">
            <div className="flex items-center gap-2">
              ...
              <Badge
                ...
                className={cn(
                  "tabular-nums",
                  accuracy.accuracy >= 90 && "text-green-600 bg-green-50",
                  accuracy.accuracy >= 70 && accuracy.accuracy < 90 && "text-yellow-600 bg-yellow-50",
                  accuracy.accuracy < 70 && "text-red-600 bg-red-50"
                )}
              >
                  ...
                  <div key={i} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-blue-600 tabular-nums min-w-[60px] text-right">
                          {formatNumber(point.forecastQuantity)}
                        </span>
                        {point.actualQuantity !== null && point.actualQuantity !== undefined && (
                          <>
                            <span className="text-emerald-600 tabular-nums min-w-[60px] text-right">
                              {formatNumber(point.actualQuantity)}
                            </span>
                            {variance !== null && (
                              <span
                                className={cn(
                                  "text-xs tabular-nums min-w-[50px]",
                      ...
                      <TableCell className="text-right">
                        {pointAccuracy !== null ? (
                          <Badge
                            ...
                            className={cn(
                              "tabular-nums",
                              pointAccuracy >= 90 && "text-green-600 bg-green-50",
                              pointAccuracy >= 70 &&
                                pointAccuracy < 90 &&
                                "text-yellow-600 bg-yellow-50",
                              pointAccuracy < 70 && "text-red-600 bg-red-50"
                            )}
                          >
