/**
 * Welcome Checklist Presenter
 *
 * ARCHITECTURE: Presenter Component - Pure UI, receives all data via props.
 *
 * Dashboard widget that displays new user onboarding progress.
 ...
 * - Permanent dismissal via callback
 * - Accessible with proper ARIA attributes
 * - Supports reduced motion preferences
 */
import { useCallback, useId, useState, useEffect, startTransition } from 'react';
...
export interface WelcomeChecklistProps {
  /** Whether the user has added their first customer */
  hasCustomer: boolean;
  /** Whether the user has added their first product */
  hasProduct: boolean;
  /** Whether the user has created their first quote */
  hasQuote: boolean;
  /** Callback to dismiss the checklist */
  onDismiss: () => void;
...
function ChecklistItemRow({ item }: ChecklistItemRowProps) {
  ...
  const linkHref =
    item.completed && item.completedHref ? item.completedHref : item.href;
  ...
  return (
    <li
      ...
      className={cn(
        ...
        item.completed ? 'bg-muted/50' : 'hover:bg-muted/30 cursor-pointer'
      )}
    >
      {/* Status icon */}
...
export function WelcomeChecklistPresenter({
  hasCustomer,
  hasProduct,
  hasQuote,
  onDismiss,
  ...
  useEffect(() => {
    if (phase1AllComplete && !celebratedPhase1) {
      startTransition(() => setCelebratedPhase1(true));
      // Small delay for celebration effect
      const timer = setTimeout(() => {
        startTransition(() => setShowPhase2(true));
      }, 500);
    ...
    >
      <CardHeader className="flex-row items-start justify-between space-y-0 pb-4">
        ...
        <div className="flex items-center gap-3">
          <ProgressRing
            progress={showPhase2 ? totalProgress : phase1Progress}
          />
          <Button
            variant="ghost"
            size="icon"
      ...
      <CardContent>
        ...
        <ul
          role="list"
          className="space-y-2"
          aria-label="Onboarding tasks phase 1"
        >
        ...
        {phase1AllComplete && showPhase2 && (
          <div className="mt-4 pt-4 border-t">
            ...
            <ul
              role="list"
              className="space-y-2"
              aria-label="Onboarding tasks phase 2"
            >
...
// Backwards compatibility alias - use WelcomeChecklistContainer in routes
export const WelcomeChecklist = WelcomeChecklistPresenter;
