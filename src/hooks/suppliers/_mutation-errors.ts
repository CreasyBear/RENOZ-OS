...
const DEFAULT_CODE_MESSAGES: Record<string, string> = {
  NOT_FOUND: 'The supplier could not be found. Refresh and try again.',
  PERMISSION_DENIED: 'You do not have permission to update suppliers.',
  AUTH_ERROR: 'Your session has expired. Sign in again before updating suppliers.',
  RATE_LIMIT: 'Too many supplier updates were attempted. Wait a moment and retry.',
  CONFLICT: 'Supplier details conflict with an existing supplier.',
};
...
export function formatSupplierMutationError(error: unknown, fallback: string): string {
