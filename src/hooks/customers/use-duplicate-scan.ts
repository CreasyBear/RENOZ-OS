...
import {
  ...
  getMergeHistory,
  type DuplicatePair,
} from '@/server/functions/customers/customer-duplicate-scan';
...
export function useDismissDuplicate() {
  ...
  return useMutation({
    mutationFn: async (data: { customer1Id: string; customer2Id: string; reason?: string }) => {
      return dismissFn({ data });
    },
    onSuccess: () => {
      // Invalidate scan results to refetch
      queryClient.invalidateQueries({ queryKey: queryKeys.customers.duplicates.all() });
    },
  });
}
 ...
 */
export function useDuplicatesPage(
  options: {
    threshold?: number;
    limit?: number;
  } = {}
