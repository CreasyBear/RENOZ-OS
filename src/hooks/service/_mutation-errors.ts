...
const DEFAULT_CODE_MESSAGES: Record<string, string> = {
  NOT_FOUND: 'Service system or linkage review could not be found. Refresh and try again.',
  PERMISSION_DENIED: 'You do not have permission to update this service workflow.',
  AUTH_ERROR: 'Your session has expired. Sign in again before updating this service workflow.',
...
function lookupCodeMessage(
  code: string,
  options: FormatServiceMutationErrorOptions
): string | undefined {
  return (
...
export function formatServiceMutationError(
  error: unknown,
  fallback: string,
  options: FormatServiceMutationErrorOptions = {}
): string {
