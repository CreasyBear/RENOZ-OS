 ...
 *
 * @see src/server/functions/dashboard/targets.ts
 * @see src/lib/schemas/dashboard/targets.ts
 */
...
export function useTargets(options: UseTargetsOptions = {}) {
  ...
  return useQuery({
    queryKey: queryKeys.dashboard.targets.list(filters),
...
export function useTarget({ id, enabled = true }: UseTargetOptions) {
  return useQuery({
    queryKey: queryKeys.dashboard.targets.detail(id),
...
export function useTargetProgress(options: UseTargetProgressOptions = {}) {
  ...
  return useQuery({
    queryKey: queryKeys.dashboard.targets.progress(filters),
...
export function useCreateTarget() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      queryClient.setQueryData(queryKeys.dashboard.targets.detail(result.id), result);
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.lists() });
      // Invalidate progress as new target affects progress calculation
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.progress() });
    },
  });
}
...
export function useUpdateTarget() {
  ...
  return useMutation({
    ...
    onSuccess: (result, variables) => {
      queryClient.setQueryData(queryKeys.dashboard.targets.detail(variables.id), result);
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.lists() });
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.progress() });
    },
  });
...
export function useDeleteTarget() {
  ...
  return useMutation({
    ...
    onSuccess: (_, id) => {
      queryClient.removeQueries({ queryKey: queryKeys.dashboard.targets.detail(id) });
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.lists() });
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.progress() });
    },
  });
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      result.targets.forEach((target) => {
        queryClient.setQueryData(queryKeys.dashboard.targets.detail(target.id), target);
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.lists() });
      queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.targets.progress() });
    },
